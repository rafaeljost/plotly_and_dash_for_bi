# example taken from https://www.youtube.com/watch?v=5U2Q60BhoWU
...
# our data
df = pd.read_csv(
    'https://raw.githubusercontent.com/Coding-with-Adam/Dash-by-Plotly/master/Analytic_Web_Apps/Flights_Analysis/europe-flights-reduced.csv')
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([html.Label("Number of Passengers Between Countries", className="align-middle")],
...
def update_graphs(year_v, month_v, geo_v, partner_v):
    ...
    if any([year_v, month_v, geo_v, partner_v]):
        ...
        psg_by_month = psg_by_month.sort_values('Value',
                                                ignore_index=True,
                                                ascending=False)
        fig_line = px.line(psg_by_month, x='MONTH', y='Value').update_xaxes(type='category').update_layout(
